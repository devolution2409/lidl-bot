...
class commandsWrapper{
	...
	init(){
		let schema = mongoose.Schema({ question: String, response: String});	
		let model
			try {
				model = mongoose.model('trivia');

			} catch (error){
				model = mongoose.model('trivia',schema);
			}

		this.initialized  = new Promise( (resolve,reject) => {
	
				var promise = () => { 
				model.find({},'answer').lean().exec( (err, answer) => {
						if (err){
						reject(err);
